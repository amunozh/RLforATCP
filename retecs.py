...
def recency_weighted_avg(values, alpha):
	return sum(np.power(alpha, list(range(0, len(values)))) * values) / len(values)
...
class PrioLearning(object):
	...
	def run_validation(self, scenario_count):
		...
		for (key, res) in list(val_res.items()):
			res = {
				'scenario': key,
				'step': scenario_count,
				'detected': res[0],
	...
	def validation(self):
		...
		return dict(list(zip(keys, results)))
	...
	def train(self, no_scenarios, print_log, plot_graphs, save_graphs, collect_comparison=True):
		...
		if collect_comparison:
			...
			for key in list(cmp_agents.keys()):
